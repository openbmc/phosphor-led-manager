...
void LampTest::getPhysicalLEDNamesFromJson(const fs::path& path)
{
    ...
    try
    {
        ...
        std::ranges::transform(forceLEDs.begin(), forceLEDs.end(),
                               std::back_inserter(forceUpdateLEDs),
                               [](const auto& i) { return PHY_LED_PATH + i; });

        auto skipLEDs = json.value("skipLEDs", empty);
        std::ranges::transform(skipLEDs.begin(), skipLEDs.end(),
                               std::back_inserter(skipUpdateLEDs),
                               [](const auto& i) { return PHY_LED_PATH + i; });
    }
