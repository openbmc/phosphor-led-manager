...
LedMap loadJsonConfig()
{
    ...
    catch (const FileError::Open& e)
    {
        log<level::ERR>("Failed to open a file", entry("ERROR=%s", e.what()),
                        entry("FILE_PATH=%s", LED_JSON_FILE));
        elog<FileError::Open>(Open::ERRNO(errno), Open::PATH(LED_JSON_FILE));
    }
    catch (const std::exception& e)
    {
        ...
        log<level::ERR>("Failed to parse a file path",
                        ...
                        entry("FILE_PATH=%s", LED_JSON_FILE));
        elog<FileError::Read>(Read::ERRNO(errno), Read::PATH(LED_JSON_FILE));
    }
