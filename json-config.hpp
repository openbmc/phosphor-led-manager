    ...
    if (iter->second != priority)
    {
        log<level::ERR>("Priority of LED is not same across all",
                        entry("Name=%s", name.c_str()),
                                  " Old Priority=std::to_string(iter->second);
                                  " New priority=std::to_string(priority));

        throw std::runtime_error("Priority of at least one LED is not same across groups");
    }
}
