...
inline Json readJson(const std::string& path)
{
    ...
    if (!fs::exists(dir) || fs::is_empty(dir))
    {
        ...
        elog<Seek>(
            xyz::openbmc_project::Common::File::Seek::OFFSET(SEEK_SET),
            xyz::openbmc_project::Common::File::Seek::WHENCE(SEEK_SET),
            xyz::openbmc_project::Common::File::Seek::ERRNO(errno),
            xyz::openbmc_project::Common::File::Seek::PATH(path.c_str()));
    }
