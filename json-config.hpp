...
class JsonConfig
{
  ...
  private:
     ...
     *     interface = Interface set in confCompatibleInterface with the
     * property property = Property set in confCompatibleProperty containing a
     * list of subdirectories in priority order to find a config
     *
                ...
                auto it =
                    std::find_if(confCompatValue.begin(), confCompatValue.end(),
                                 [this](auto const& entry) {
                                     this->confFile = fs::path{confBasePath} /
                                                      entry / confFileName;
                                     return fs::exists(this->confFile);
                                 });
