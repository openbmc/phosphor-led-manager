                      ...
                      PriorityMap& priorityMap)
{
    ...
    if (iter->second != priority)
    {
        throw std::runtime_error(
            "the Priority of the same name is not the "
            "same across ALL groups, name= " +
            name + ", previous priority = " + std::to_string(priority) +
            ", current priority= " + std::to_string(iter->second));
    }
}
