    ...
    Monitor::getLedGroupPath(const std::string& inventoryPath) const
{
    ...
    try
    {
        endpoint = dBusHandler.getProperty(
            faultLedGroupPath, "xyz.openbmc_project.Association", "endpoints");
    }
    catch (const sdbusplus::exception::SdBusError& e)
    {
        log<level::ERR>("Failed to get endpoints property",
                        entry("ERROR=%s", e.what()),
                        entry("PATH=%s", faultLedGroupPath.c_str()));
    ...
    return endpoints.empty() ? std::string{} : endpoints[0];
}
                                     ...
                                     bool value)
{
    try
    {
        ...
        // Call "Group Asserted --> false" if the value of Functional is true
        PropertyValue assertedValue{!value};
        dBusHandler.setProperty(ledGroupPath, "xyz.openbmc_project.Led.Group",
    ...
    catch (const sdbusplus::exception::SdBusError& e)
    {
        log<level::ERR>("Failed to set Asserted property",
                        entry("ERROR=%s", e.what()),
                        entry("PATH=%s", ledGroupPath.c_str()));
    }
}
} // namespace monitor
} // namespace status
