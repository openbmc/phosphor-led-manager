...
class Monitor
{
  ...
  private:
     ...
     *
     * @param[in] inventoryPath         - The inventory path of the Inventory
     *                                    D-Bus object
     *
     * @return std::vector<std::string> - The object paths of the LED
     *                                    GroupManager
     */
    const std::vector<std::string>
        getLedGroupPath(const std::string& inventoryPath) const;

    /**
     * @brief Update the Asserted property of the LED Group Manager.
     *
     * @param[in] ledGroupPath    - The led group paths of the LED Group Manager
     * @param[in] value           - the Asserted value, True / False
     */
    void updateAssertedProperty(const std::vector<std::string>& ledGroupPath,
                                bool value);
};
} // namespace monitor
} // namespace status
