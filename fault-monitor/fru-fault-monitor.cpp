...
using MethodErr  = xyz::openbmc_project::Led::Mapper::MethodError;
                       ...
                       const std::string& path)
{
    ...
    {
        elog<MethodErr>(
            MethodErr::METHOD_NAME("GetObject"),
            MethodErr::PATH(path.c_str()),
            MethodErr::INTERFACE(
                OBJMGR_IFACE));
    }
    ...
    if (mapperResponse.empty())
    {
        elog<ObjectNotFoundErr>(
            ObjectNotFoundErr::METHOD_NAME("GetObject"),
            ObjectNotFoundErr::PATH(path.c_str()),
            ObjectNotFoundErr::INTERFACE(
                OBJMGR_IFACE));
    }
                 ...
                 sd_bus_error* retError)
{
    ...
    if (reply.is_method_error())
    {
        report<AssociationRetrieveErr>(
            AssociationRetrieveErr::ELOG_ENTRY_PATH(
                objectPath.c_str()));
        return 0;
    }
