                       ...
                       const std::string& path)
{
    ...
    {
        std::string err = "Error in GetObject from mapper, path:"+path;
        elog<xyz::openbmc_project::fru::fault::Monitor::LedFailure>(
            xyz::openbmc_project::fru::fault::Monitor::LedFailure::Error(err.c_str()));
    }
    ...
    {
        std::string err = "Error in getting Mapper Response, Path:"+path;
        elog<xyz::openbmc_project::fru::fault::Monitor::LedFailure>(
            xyz::openbmc_project::fru::fault::Monitor::LedFailure::Error(err.c_str()));
    }
            ...
            const bool assert)
{
    ...
    catch(xyz::openbmc_project::fru::fault::Monitor::LedFailure e)
    {
        commit(e.name());
        return;
    }
    ...
    if (pos == std::string::npos)
    {
        ...
        report<xyz::openbmc_project::fru::fault::Monitor::LedFailure>(
            xyz::openbmc_project::fru::fault::Monitor::LedFailure::Error(err.c_str()));
        return;
    }
                 ...
                 sd_bus_error* retError)
{
    ...
    {
        report<xyz::openbmc_project::fru::fault::Monitor::LedFailure>(
            xyz::openbmc_project::fru::fault::Monitor::LedFailure::Error("Error in getting associations from error entry"));
        return 0;
    }
