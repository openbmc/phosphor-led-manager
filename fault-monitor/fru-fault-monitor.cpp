...
std::string getService(sdbusplus::bus::bus& bus, const std::string& path)
{
    ...
    {
        log<level::ERR>(fmt::format("Failed to parse getService mapper "
                                    "response, errormsg = {}, REPLY_SIG = {}",
...
void action(sdbusplus::bus::bus& bus, const std::string& path, bool assert)
{
    ...
    catch (const sdbusplus::exception::SdBusError& e)
    {
        ...
        log<level::INFO>(
            fmt::format("Failed to Assert LED Group, errormsg = {}", e.what())
                .c_str());
    }
...
void Add::created(sdbusplus::message::message& msg)
{
    ...
    {
        log<level::ERR>(fmt::format("Failed to parse created message, errormsg "
                                    "= {}, REPLY_SIG = {}",
    ...
    log<level::INFO>(fmt::format("message = {}", message).c_str());
...
void getLoggingSubTree(sdbusplus::bus::bus& bus, MapperResponseType& subtree)
{
    ...
    try
    {
        ...
        {
            log<level::ERR>(
                fmt::format("Failed to parse existing callouts subtree "
                            "message, errormsg = {}, REPLY_SIG = {}",
                            e.what(), mapperResponseMsg.get_signature())
                    .c_str());
        }
    }
...
void Add::processExistingCallouts(sdbusplus::bus::bus& bus)
{
    ...
    for (const auto& elem : mapperResponse)
    {
        ...
        {
            log<level::ERR>(
                fmt::format("Failed to parse existing callouts associations "
                            "message, errormsg = {}, REPLY_SIG = {}",
                            e.what(), reply.get_signature())
                    .c_str());
            continue;
        }
