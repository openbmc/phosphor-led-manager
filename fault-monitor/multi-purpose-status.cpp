...
void Status::sled()
{
    size_t maxPosition = getPropertyValue<size_t>(
        SELECTOR_OBJPATH, SELECTOR_INTERFACE, "MaxPosition");

    for (size_t pos = 1; pos < maxPosition; pos++)
    {
...
emplate<typename T> T Status::getPropertyValue(const std::string& objectPath,
...
void Status::matchHandler(sdbusplus::bus::bus& bus, const std::string& config)
{
    ...
    if (config == "DebugCard")
    {
        matchSignal = std::make_unique<sdbusplus::bus::match_t>(
            bus,
            sdbusplus::bus::match::rules::propertiesChanged(SELECTOR_OBJPATH,
                                                            SELECTOR_INTERFACE),
