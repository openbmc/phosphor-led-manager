...
void LampTest::start()
{
    ...
    // place.
    const fs::path lamptestPath = LAMP_TEST_INDICATOR_FILE;
    const auto lamptestDirectory = lamptestPath.parent_path();

    if (!fs::exists(lamptestDirectory))
    {
        fs::create_directories(lamptestDirectory);
    }
