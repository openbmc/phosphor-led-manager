...
static void validateConfigV1Group(const std::string& groupName,
                                  const Layout::GroupLayout& group)
{
    // this check is valid for both configuration styles
    std::set<std::string> ledNames;
    for (const auto& ledAction : group.actionSet)
    {
        if (ledNames.contains(ledAction.name))
        {
            throw ConfigValidationException(error::DuplicateLedInGroup,
                                            groupName, ledAction.name,
                                            "Duplicate LED defined in group.");
        }
        ledNames.insert(ledAction.name);
    }
}
...
void validateConfigV1(const GroupMap& ledMap)
{
    ...
    for (const auto& [groupName, group] : ledMap)
    {
        validateConfigV1Group(groupName, group);
    }
}
