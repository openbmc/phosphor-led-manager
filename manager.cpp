                               ...
                               const uint16_t period)
{
    try
    {
        ...
        if (action == Layout::Action::Blink)
        {
            ...
            dBusHandler.setDbusProperty(objPath, PHY_LED_IFACE, "DutyOn",
                                        dutyOnValue);
            dBusHandler.setDbusProperty(objPath, PHY_LED_IFACE, "Period",
                                        periodValue);
        }

        PropertyValue actionValue{getPhysicalAction(action)};
        dBusHandler.setDbusProperty(objPath, PHY_LED_IFACE, "State",
                                    actionValue);
    }
