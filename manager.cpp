...
PropertyValue Manager::getPhysicalLED(const std::string& objectPath,
                                      const std::string& propertyName)
{
    PropertyValue value;
    try
    {
        value = dBusHandler->getProperty(objectPath.c_str(),
                                         propertyName.c_str(), PHY_LED_IFACE);
    }
    catch (const std::exception& e)
    {
        log<level::ERR>("Error getting property for physical LED",
                        entry("ERROR=%s", e.what()),
                        entry("FILE_PATH=%s", objectPath.c_str()),
                        entry("PROPERTY_NAME=%s", propertyName.c_str()));
    }
