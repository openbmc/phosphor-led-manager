                               ...
                               const uint16_t period)
{
    try
    {
        ...
        {
            DBusMapping dutyOnMapping{objPath, PHY_LED_IFACE, "DutyOn"};
            DBusMapping periodMapping{objPath, PHY_LED_IFACE, "Period"};

            PropertyValue dutyOnValue{dutyOn};
            PropertyValue periodValue{period};

            DBusHandler().setDbusProperty(dutyOnMapping, dutyOnValue);
            DBusHandler().setDbusProperty(periodMapping, periodValue);
        }

        DBusMapping actionMapping{objPath, PHY_LED_IFACE, "State"};
        PropertyValue actionValue{getPhysicalAction(action)};

        DBusHandler().setDbusProperty(actionMapping, actionValue);
    }
    catch (const std::exception& e)
    {
        log<level::ERR>("Error setting property for physical LED",
                        entry("ERROR=%s", e.what()),
                        entry("FILE_PATH=%s", objPath.c_str()));
    }
