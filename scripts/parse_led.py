...
def config_error(ofile, led_name, message):
    ofile.close()
    os.remove(ofile.name)
    raise ValueError(
        "Invalid Configuration for LED [" + led_name + "]: " + message
    )
...
def check_led_priority(led_name, value, priority_dict):
    ...
    if led_name in priority_dict:
        if value != priority_dict[led_name]:
            ...
            # across all groups
            config_error(
                ofile, led_name, "Priority is NOT same across all groups"
            )
    else:
...
def generate_file_single_led(
    ...
    if has_group_priority and has_led_priority:
        config_error(
            ofile, led_name, "cannot mix group priority with led priority"
        )

    if (not has_group_priority) and (not has_led_priority):
        config_error(
            ofile, led_name, "no group priority or led priority defined"
        )

    led_priority = list_dict.get("Priority", "")

    if has_led_priority:
        check_led_priority(led_name, led_priority, priority_dict)
